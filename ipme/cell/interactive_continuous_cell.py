...
from ..utils.constants import BORDER_COLORS, PLOT_HEIGHT, PLOT_WIDTH, SIZING_MODE, RUG_DIST_RATIO, RUG_SIZE
...
class InteractiveContinuousCell(VariableCell):
    ...
    def initialize_cds(self, space):
        ...
        self._samples[space] = ColumnDataSource(data = dict( x = samples, y = np.asarray([-max_v/RUG_DIST_RATIO]*len(samples)), size = np.asarray([RUG_SIZE]*len(samples))))
        self._sel_samples[space] = ColumnDataSource(data = dict(x = np.array([]), y = np.array([]), size = np.array([])))
    ...
    def _update_source_cds(self, space):
        CellContinuousHandler.update_source_cds_interactive_continuous(self, space)

    def _update_selection_cds(self, space, xmin, xmax):
        CellContinuousHandler.update_selection_cds_interactive_continuous(self, space, xmin, xmax)

    def _update_reconstructed_cds(self, space):
        CellContinuousHandler.update_reconstructed_cds_interactive_continuous(self, space)

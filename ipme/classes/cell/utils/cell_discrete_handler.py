...
class CellDiscreteHandler:
    ...
    def initialize_glyphs(variableCell, space):
        ...
        so_scat = variableCell.plot[space].scatter('x', 'y', source = variableCell.source[space], size = 4, fill_color = COLORS[0], fill_alpha = 1.0, line_color = COLORS[0], selection_fill_color = COLORS[0], \
        ...
        variableCell.plot[space].segment(x0 = 'x', y0 ='y0', x1 = 'x', y1 = 'y', source = variableCell.reconstructed[space], line_alpha = 0.5, color = COLORS[1], line_width = 1)
        ...
        return (so_seg, so_scat)

    @staticmethod
    def initialize_glyphs_interactive(variableCell, space):
        so_seg,so_scat = CellDiscreteHandler.initialize_glyphs(variableCell, space)
        ##Add BoxSelectTool
        variableCell.plot[space].add_tools(BoxSelectTool(dimensions = 'width', renderers = [so_seg,so_scat]))
        ##########TEST###################
        TOOLTIPS = [("x", "@x"), ("y","@y"),]
        hover = HoverTool( tooltips = TOOLTIPS, renderers = [so_seg,so_scat], mode = 'mouse')
    ...
    def update_selection_cds_interactive(variableCell, space, xmin, xmax):
        ...
        data['x'] = np.array([])
        data['y'] = np.array([])
        kde_indices = find_indices(variableCell.source[space].data['x'], lambda e: e >= xmin and e<= xmax,xmin,xmax)

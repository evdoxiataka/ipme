...
class Grid(ABC):
    ...
    #                 if w_id in self.cells_widgets:
    #                     if space in self.cells_widgets[w_id]:
    #                         self.cells_widgets[w_id][space].append(c_id)
    #                     else:
    #                         self.cells_widgets[w_id][space] = [c_id]
    ...
    #                     f_space = list(self.cells_widgets[w_id].keys())[0]
    #                     self._link_widget_to_target(w, w_id, f_space)
    #                 else:
    #                     self.cells_widgets[w_id] = {}
    #                     self.cells_widgets[w_id][space] = [c_id]
    #                     f_space = list(self.cells_widgets[w_id].keys())[0]
    #                     if f_space != space:
    #                         self._link_widget_to_target(w, w_id, f_space)
    #                     else:
    #                         w = self.cells[c_id].get_widget(space, w_id)
    #                         w.on_change('value', partial(self.ic.menu_item_click_callback, self.cells, self.cells_widgets, space, w_id))
    #
    # def _link_widget_to_target(self, w, w_id, f_space):
    #     if len(self.cells_widgets[w_id][f_space]):
    #         t_c_id = self.cells_widgets[w_id][f_space][0]
    #         t_w = self.cells[t_c_id].get_widget(f_space, w_id)
    #         if t_w is not None and hasattr(t_w,'js_link'):
    #             t_w.js_link('value', w, 'value')
    #
    # def _set_plotted_widgets(self):
    #     self.plotted_widgets = {}
    #     for w_id, space_widgets_dict in self.cells_widgets.items():
    ...
    #                     if space not in self.plotted_widgets:
    #                         self.plotted_widgets[space] = {}
    #                     self.plotted_widgets[space][w_id] = self.cells[c_id].get_widget(f_space, w_id)
    #     b = Button(label='Reset Diagram', button_type="primary")
    #     b.on_click(self._global_reset_callback)
    #     for space in self.spaces:
    #         if space not in self.plotted_widgets:
    #             self.plotted_widgets[space] = {}
    #         self.plotted_widgets[space]["resetButton"] = b

    # def _global_reset_callback(self, event):
    #     self.ic.reset_sel_var_inds()
    #     self.ic.reset_sel_space()
    #     self.ic.reset_sel_var_idx_dims_values()
    #     self.ic.reset_var_x_range()
    #     self.ic.set_global_update(True)
    #     for sp in self.spaces:
    #         self.ic.add_space_threads(threading.Thread(target=partial(self._global_reset_thread, sp), daemon=True))
    #     self.ic.space_threads_join()
    #     self.ic.set_global_update(False)
    #
    # def _global_reset_thread(self,space):
    #     self.ic.reset_sample_inds(space)
